...
# def is_applicable_old(state, action):
...
def excuteModelProgress(GenCode,init,exam,is_root):
        ...
        while j < len(GenCode) and runline < len(exam[i]):
            ...
            if((GenCode[j].flag=="Loop" or (GenCode[j].flag=="IF" and IfExecute==False)) and isSimpleLoop(GenCode[j])==False):
                ...
                while contExecute==True and runline<len(exam[i]):
                    ...
                    preIsFullExecute=copy.deepcopy(isFullExecute)
                    # prerunline=runline
                    #pState(preState)
                    #1.san,2.two.
                    ...
                    GenCode[j].actionList,runline1,runExam,contExecute,curIsFullExecute=excuteModelProgress(GenCode[j].actionList,[runExam],[exam[i][runline:]],is_root+1)
                    #print("The final res:"+str(contExecute))
                    #res=runExam
                    # subRegex accepts the subsequence --- this state is added in the positive state
                    #匹配成功
                    #print(str(curIsFullExecute))
                    if(contExecute and curIsFullExecute):
                        lastExcute=True
                        hasExecute=True
                        inputs={}
                        #add 20221206
                        runStateslist.append({'State': preState, 'RegexIndex': j, 'CaseIndex':runline, 'preGenCode':preGenCode,'preIsFullExecute':preIsFullExecute})
                    ...
                    if GenCode[j].flag == "IF":
                        break
                isFullExecute=isFullExecute  and lastExcute
                ...
                elif (GenCode[j].flag == "IF" and contExecute == False  or (contExecute==True and curIsFullExecute==False)):
                    IfExecute = False
            ...
            # simple loop or simple if or seq statement ----
                ...
                if(GenCode[j].flag=="Loop"):
                    ...
                    if(loopBody==pattern):
                        ...
                        #add 20221206
                        runStateslist.append({'State': copy.deepcopy(runExam), 'RegexIndex': j, 'CaseIndex':runline,'preGenCode':copy.deepcopy(GenCode),'preIsFullExecute':copy.deepcopy(isFullExecute)})
                        for item in GenCode[j].actionList:
                            if is_applicable(runExam,actionList[item.actionList[0]]):
                                ...
                                break
                        if isCanApply==True:
                            isLoopAlreadyExecute=True
                            runline=runline+step
                            if j+1== len(GenCode) and runline == len(exam[i]):
                                GenCode[j].examNeg.add(copy.deepcopy(runExam))
                                j=j+1
                        else:
                            GenCode[j].examNeg.add(copy.deepcopy(runExam))
                            j=j+1
                            isFullExecute=isFullExecute and isLoopAlreadyExecute
                            isLoopAlreadyExecute=False
                    else:
                        GenCode[j].examNeg.add(copy.deepcopy(runExam))
                        j=j+1
                ...
                #
                    ...
                    tmp_runline = runline
                    tmp_isFullExecute=isFullExecute
                        ...
                        if isCanApply==True:
                            runStateslist.append(
                                {'State': tmp_runExam, 'RegexIndex': tmp_j, 'CaseIndex': tmp_runline,
                                 'preGenCode':tmp_GenCode ,'preIsFullExecute':tmp_isFullExecute})
                    ...
                    if(seqBody==pattern):
                        ...
                        if isCanApply==True:
                            ...
                            if len(runStateslist)>0:
                                ...
                                isFullExecute=backState['preIsFullExecute']
                                IfExecute=False
                                ...
                                isFullExecute=isFullExecute and False
                                break
                    else:
                        hasExecute=False
                        #return tmpGenCode,runline,'',hasExecute
        ...
        if(j < len(GenCode) and runline==len(exam[i]) and ifMatchDone(GenCode,j) == True) :
            ...
            while temp < len(GenCode):
                if GenCode[temp].flag in ('IF', 'IFe'):
                    SeqAndLoop_State and GenCode[temp].examNeg.add(copy.deepcopy(SeqAndLoop_State))
                    runExam and GenCode[temp].examNeg.add(copy.deepcopy(runExam))
                else:
                    GenCode[temp].examNeg.add(copy.deepcopy(runExam))
                temp += 1
    #print(str(hasExecute)+";"+str(isFullExecute)+"j:"+str(j)+";runline:"+str(runline))
    if hasExecute == False:
    ...
    return GenCode,runline,runExam,hasExecute,isFullExecute
...
def complete2(GenCode,domainfile,problemfile,planExamples):
    # 1. get action list
    ...
    GenCode, runline, runExam, hasExecute,isFullExecute = excuteModelProgress(GenCode, init, planExamples, 0)
